...
`tools/list` at `https://gateway.pipeworx.io/datacube-sk/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/datacube_sk_list_datasets \
  -H 'Content-Type: application/json' \
  -d '{"query":"population"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/datacube_sk_list_datasets`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "datacube-sk": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-datacube-sk"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-datacube-sk
```
